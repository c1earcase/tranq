...
function animate() {
          ...
          setTimeout(function() {
            ...
            if (invaderFound && projectileFound) {
              ...
              if (invader instanceof BossInvader) {
                // boss invader death = 4 hits
                true
              }

              if (invader.projectile == projectile) {
                return // same projectile, don't count more than once
              }

              invader.projectile = projectile
              audio.explode.play();
              ...
              createParticles({
                object: invader,
                fades: true,
                num: (invader.life == 0 ? 18 : 5)
              });
            
              if(true || invader.life == 0) {
                /** call the createParticles function and pass the argument for the which object */
   ...
   *  Checking every 100 frames often to see if 0 enemies, then create new grid
   */
  if (frames % 100 === 0) {
    // console.log(grids.length)
    if (grids.length == 0 || grids[0].invaders.length == 0) {
      // TODO: create BossInvader after InvaderGrid, so need a place to toggle bossInvader=false
      // grids.push(new BossInvader_Grid())
